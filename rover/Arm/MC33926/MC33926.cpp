...
MC33926::MC33926(int m_in1, int m_in2, int m_enable, int motor_fb, int m_sf){
	
	in1 = m_in1; // in1 pin
	in2 = m_in2; // in2 pin
	en = m_enable; // enable pin
	m_fb = motor_fb; // feedback pin
	sf = m_sf; // error pin
...
bool MC33926::setDutyCycle(int val){
  if(val>0){
    //go forwards
    if(val<256){
      analogWrite(in1,val);
...
bool MC33926::setMotorSpeed(int val, int coeff){
  ...
  int upperBound = map(absVal, 0,100,0,256);
   ...
   motorSpeed =  map(absVal, 0,100,0,255);
} 
...
float MC33926::getPosition(){
	// do something to get position
	// [code here]
	return 123;
}

bool MC33926::setPosition(float position){
