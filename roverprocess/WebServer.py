...
class WebServer(RoverProcess):
	...
	def startBottleServer(self):
		localRoutes = WebServerRoutes(parent=self, dataSem=self.dataSem)
		run(host='localhost', port=8000, server=GeventWebSocketServer, app=localRoutes.instance, debug=True)
